...
if __name__ == "__main__":
    ...
    experiments = [10,25,50,75,100,125,150,200,300,400,500]

    for numFrames in experiments:
        print(video)

        ##reduce to directory
        directory = os.path.dirname(video)+ str (numFrames)
        ...
        if "colmap" not in os.listdir(directory):
            os.system(
                f"ns-process-data video --data {video} --output-dir {directory} --matching-method sequential --verbose --num-frames-target {str (numFrames)} "
            )
