...
def epoch_division_Test_sphere():
    ...
    for e in epochs:
        for d in divisions:
            os.system(
                        f"ns-export Marching-tet --load-config {configDir} --output-dir exports/sphere_epoch_test/Epoch{e}_Div{d}_{timestring}/ --use-bounding-box True --bounding-box-min -0.004000000000000031 -0.25200000000000006 -0.11099999999999997 --bounding-box-max 0.30400000000000005 0.04799999999999993 0.189 --epochs {e} --batch_splits {d} --loss_weights 0.01 0.00001 0.00001 0.00001 --nerf_image_path=ssan/testsphere"
                    )
