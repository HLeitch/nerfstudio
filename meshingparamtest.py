        ...
        for l2 in loss2:
            for l3 in loss3:
                for l4 in loss4:
                    arr = f"{l1} {l2} {l3} {l4}"
                    os.system(
                        f"ns-export Marching-tet --load-config {configDir} --output-dir exports/skmcTest/{l1}_{l2}_{l3}_{l4}/ --use-bounding-box True --bounding-box-min -0.15 -0.2 -0.35 --bounding-box-max 0.15 0.2 0.35 --epochs 15 --batch_splits 150 --loss_weights {l1} {l2} {l3} {l4} --nerf_image_path=ssan/teststatue --resolution 256 256 256"
                    )
...
def epoch_division_Test_statue():
    ...
    for e in epochs:
        for d in divisions:
            os.system(
                        f"ns-export Marching-tet --load-config {configDir} --output-dir exports/statue_hires_epoch_test/Epoch{e}_Div{d}/ --use-bounding-box True --bounding-box-min -0.15 -0.2 -0.35 --bounding-box-max 0.15 0.2 0.35 --epochs {e} --batch_splits {d} --loss_weights 0.01 0.0001 0.0001 0.001 --nerf_image_path=ssan/teststatue"
                    )
